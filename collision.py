...
class Particle:
    def __init__(self):
        self.position = [random.uniform(-0.4, 0.4) for _ in range(3)]  # Adjusted range to fit inside the cube
    ...
    def update(self, gravity_on):
        for i in range(3):
            ...
            if abs(self.position[i]) >= 0.5:  # Changed from 1 to 0.5 for cube collision
                self.velocity[i] *= -1
        if gravity_on:
